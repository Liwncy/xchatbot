...
玩法存档已迁到 `cf-mcp-tools`。本仓库 `#` 快捷口令集中在一处：

```text
src/plugins/command/mcp-tools/
├── index.ts              # 插件入口
├── catalog.ts            # 加工具 / 加 MCP 入口只改这里
├── dispatch.ts           # 匹配 → 调 MCP → HandlerResponse
└── keywords.ts           # 关键词表（玄学等，以后也往这儿加）
```
...
| mcp-tools | command | S | `command/mcp-tools/` | `#` 快捷调用全部 CF MCP 工具 |
